...
const line2 = new Connection(new SpacePoint(4, 0, 0), new SpacePoint(4, 2000, 0));
const dividingLines = [];
...
const activeItems = [line1, line2, ...dividingLines, ...cubes, pickupTruck];
...
function atKeyPress(e){
    if (e.key == 'ArrowLeft') {
        clearCanvas();
        activeItems.forEach(el =>  el.move(LEFT));
    } else if (e.key == 'ArrowRight') {
        clearCanvas();
        activeItems.forEach(el => el.move(RIGHT));
    } else if (e.code == 'KeyC') {
        clearCanvas();
        activeItems.forEach(el => el.move(CLOSER));
    } else if (e.code == 'KeyF') {
        clearCanvas();
        activeItems.forEach(el => el.move(FAR));
    }
}
